  ...
  let color = db.get(`${process.env.owner}.color`) 
   if(color === null  ) color = process.env.color
        ...
        if(process.env.owner ===message.author.id || db.get(`ownermd.${message.author.id}`) === true || db.get(`${message.guild.id}.${message.author.id}.wlmd`) === true) {
            let tdata = await message.channel.send(new MessageEmbed().setColor(color).setTitle(`Liste des bot admins présents`))
        ...
        if(str_filtrer.size === 0 || undefined || false || null) return message.channel.send(new Discord.MessageEmbed().setColor(color).setTitle("Aucun bot admins présents"))
            ...
            embed.setTitle(`Liste des bot admins présents`)
            ...
            .setColor(color)
            .setFooter(`Total: ${str_filtrer.size} • Mprotect`)
